                                 ...
                                 gpointer user_data)
{
    ...
    {
        GtkWidget *combobox;
        combobox = lookup_widget(togglebutton, "bv_combobox");
        gtk_combo_box_append_text(GTK_COMBO_BOX(combobox), NULL);
    }
    else
    {

        int bv[50][10] = {1, 2, 3, 4, 5, 6};
        GtkWidget *combobox;
        combobox = lookup_widget(togglebutton, "bv_combobox");
        /* n = tableau_bv_disponible();*/
        for (int i = 0; i < 6; i++)
        {
            gtk_combo_box_append_text(GTK_COMBO_BOX(combobox), (bv[i]));
        }
    }
}
