#include <gtk/gtk.h>

void on_RG_radiobuttonH_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_RG_radiobuttonT_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_RG_radiobuttonAutre_toggled(GtkToggleButton *togglebutton,
                                    gpointer user_data);

void on_RG_radiobuttonC_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_RG_radiobuttonD_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_RG_radiobuttonM_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_RG_checkbuttonA_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_RG_checkbuttonE_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_RG_buttonACC_clicked(GtkWidget *button,
                             gpointer user_data);

void on_RG_buttonAJ_clicked(GtkWidget *objet_graphique,
                            gpointer user_data);

void on_RG_buttonAff_clicked(GtkWidget *button,
                             gpointer user_data);

void on_buttonCV_clicked(GtkWidget *button,
                         gpointer user_data);

void on_SR_buttonAV_clicked(GtkWidget *button,
                            gpointer user_data);

void on_SR_buttonML_clicked(GtkWidget *button,
                            gpointer user_data);

void on_SRbuttonSL_clicked(GtkWidget *button,
                           gpointer user_data);

void on_SR_buttonCHER_clicked(GtkWidget *button,
                              gpointer user_data);

void on_buttonPRE2_clicked(GtkWidget *button,
                           gpointer user_data);

void on_droitebutton_toggled(GtkToggleButton *togglebutton,
                             gpointer user_data);

void on_SR_radiobuttonG_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_centrebutton_toggled(GtkToggleButton *togglebutton,
                             gpointer user_data);

void on_buttonCA_clicked(GtkWidget *button,
                         gpointer user_data);

void on_SR_buttonPR_clicked(GtkWidget *button,
                            gpointer user_data);

void on_SR_afficherListe_clicked(GtkWidget *button,
                                 gpointer user_data);

void on_RG_checkbuttonHote_toggled(GtkToggleButton *togglebutton,
                                   gpointer user_data);

void on_RG_checkbuttonObservateur_toggled(GtkToggleButton *togglebutton,
                                          gpointer user_data);

void on_RG_checkbuttonInterprete_toggled(GtkToggleButton *togglebutton,
                                         gpointer user_data);

void on_RG_checkbuttonJournalist_toggled(GtkToggleButton *togglebutton,
                                         gpointer user_data);

void on_RG_checkbuttonAmbassadeur_toggled(GtkToggleButton *togglebutton,
                                          gpointer user_data);

void on_RG_checkbuttonActiviste_toggled(GtkToggleButton *togglebutton,
                                        gpointer user_data);

void on_RG_button2ACC_clicked(GtkWidget *button,
                              gpointer user_data);

void on_RG_button2A_clicked(GtkWidget *objet_graphique,
                            gpointer user_data);

void on_RG_buttonA1_clicked(GtkWidget *button,
                            gpointer user_data);

void on_RG_buttonC_clicked(GtkWidget *button,
                           gpointer user_data);

void on_RG_buttonAnnulerSuppU_clicked(GtkWidget *button,
                                      gpointer user_data);

void on_RG_buttonconfirmerSupprission_clicked(GtkWidget *button,
                                              gpointer user_data);

void on_OZ_button29_clicked(GtkWidget *button,
                            gpointer user_data);

void on_OZ_button28_clicked(GtkWidget *button,
                            gpointer user_data);

void on_OZ_treeview_row_activated(GtkTreeView *treeview,
                                  GtkTreePath *path,
                                  GtkTreeViewColumn *column,
                                  gpointer user_data);

void on_OZ_button30_clicked(GtkWidget *button,
                            gpointer user_data);

void on_OZ_ajouterRec_clicked(GtkWidget *button,
                              gpointer user_data);

void on_OZ_modifierRec_clicked(GtkWidget *button,
                               gpointer user_data);

void on_OZ_chercherRec_clicked(GtkWidget *button,
                               gpointer user_data);

void on_OZ_actualiser_clicked(GtkWidget *button,
                              gpointer user_data);

void on_OZ_supprimerRec_clicked(GtkWidget *button,
                                gpointer user_data);

void on_OZ_button34_clicked(GtkWidget *button,
                            gpointer user_data);

void on_OZ_radiobutton1_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_OZ_radiobutton4_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_OZ_radiobutton3_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_OZ_button35_clicked(GtkWidget *button,
                            gpointer user_data);

void on_OZ_button37_clicked(GtkWidget *button,
                            gpointer user_data);

void on_button36_clicked(GtkWidget *button,
                         gpointer user_data);

void on_OZ_radiobuttonc_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_OZ_radiobuttona_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_OZ_radiobuttonb_toggled(GtkToggleButton *togglebutton,
                                gpointer user_data);

void on_OZ_button39_clicked(GtkWidget *button,
                            gpointer user_data);

void on_OZ_button38_clicked(GtkWidget *button,
                            gpointer user_data);

void on_FSBVbutton_ACC_RELOAD_BV_clicked(GtkWidget *objet,
                                         gpointer user_data);

void on_FSBVbutton_ACC_AJ_BV_clicked(GtkWidget *button,
                                     gpointer user_data);

void on_FSBVbutton_ACC_SUPP_BV_clicked(GtkWidget *button,
                                       gpointer user_data);

void on_FSBVbutton_ACC_MOD_BV_clicked(GtkWidget *button,
                                      gpointer user_data);

void on_FSBVradiobutton_AJ_CPT_ELEC1_BV_toggled(GtkToggleButton *togglebutton,
                                                gpointer user_data);

void on_FSBVradiobutton_AJ_CPT_ELEC2_BV_toggled(GtkToggleButton *togglebutton,
                                                gpointer user_data);

void on_FSBVradiobutton_AJ_CPT_ELEC3_BV_toggled(GtkToggleButton *togglebutton,
                                                gpointer user_data);

void on_FSBVbutton_AJ_ANNULER_BV_clicked(GtkWidget *button,
                                         gpointer user_data);

void on_FSBVbutton_AJ_AJ_BV_clicked(GtkWidget *button,
                                    gpointer user_data);

void on_FSBVcheckbutton_AJ_CHECK_BV_toggled(GtkToggleButton *togglebutton,
                                            gpointer user_data);

void on_FSBVradiobutton_MOD_CPT_ELEC1_BV_toggled(GtkToggleButton *togglebutton,
                                                 gpointer user_data);

void on_FSBVradiobutton_MOD_CPT_ELEC2_BV_toggled(GtkToggleButton *togglebutton,
                                                 gpointer user_data);

void on_FSBVradiobutton_MOD_CPT_ELEC3_BV_toggled(GtkToggleButton *togglebutton,
                                                 gpointer user_data);

void on_FSBVbutton_MOD_MODIFIER_BV_clicked(GtkWidget *button,
                                           gpointer user_data);

void on_FSBVbutton_MOD_ANNULER_BV_clicked(GtkWidget *button,
                                          gpointer user_data);

void on_FSBVbutton_SUPP_NON_BV_clicked(GtkWidget *button,
                                       gpointer user_data);

void on_RG_treeviewUser_row_activated(GtkTreeView *treeview,
                                      GtkTreePath *path,
                                      GtkTreeViewColumn *column,
                                      gpointer user_data);

void on_RG_buttonAjouter_clicked(GtkWidget *button,
                                 gpointer user_data);

void on_RG_buttonModifier_clicked(GtkWidget *button,
                                  gpointer user_data);

void on_RG_buttonRetour_clicked(GtkWidget *button,
                                gpointer user_data);

void on_RG_buttonActualiser_clicked(GtkWidget *objet,
                                    gpointer user_data);

void on_RG_buttonSupprimer_clicked(GtkWidget *button,
                                   gpointer user_data);

void on_RG_ButtonEA_clicked(GtkWidget *button,
                            gpointer user_data);

void on_RG_buttonER_clicked(GtkWidget *button,
                            gpointer user_data);

void on_RG_buttonEE_clicked(GtkWidget *button,
                            gpointer user_data);

void on_RG_buttonEBV_clicked(GtkWidget *button,
                             gpointer user_data);

void on_RG_buttonStats_clicked(GtkWidget *button,
                               gpointer user_data);

void on_RG_buttonAccueil_clicked(GtkWidget *button,
                                 gpointer user_data);

void on_RG_ButtonGBV_clicked(GtkWidget *button,
                             gpointer user_data);

void on_RG_buttonGE_clicked(GtkWidget *button,
                            gpointer user_data);

void on_RG_buttonGU_clicked(GtkWidget *button,
                            gpointer user_data);

void on_RG_buttonAgeMoy_clicked(GtkWidget *objet_graphique,
                                gpointer user_data);

void on_RG_buttonnbob_clicked(GtkWidget *objet_graphique,
                              gpointer user_data);

void on_RG_buttonTauxobs_clicked(GtkWidget *objet_graphique,
                                 gpointer user_data);

void on_RG_buttonSRetour_clicked(GtkWidget *button,
                                 gpointer user_data);

void on_RG_buttonSAccueil_clicked(GtkWidget *button,
                                  gpointer user_data);

void on_radiobuttonNewCel_toggled(GtkToggleButton *togglebutton,
                                  gpointer user_data);

void on_RG_radiobuttonNewDiv_toggled(GtkToggleButton *togglebutton,
                                     gpointer user_data);

void on_RG_radiobuttonNewMarie_toggled(GtkToggleButton *togglebutton,
                                       gpointer user_data);

void on_RG_checkbuttonNewE_toggled(GtkToggleButton *togglebutton,
                                   gpointer user_data);

void on_RG_buttonModUtilisateur_clicked(GtkWidget *button,
                                        gpointer user_data);

void on_RG_buttonAcceuilMod_clicked(GtkWidget *button,
                                    gpointer user_data);

void on_RG_buttonMODAff_clicked(GtkWidget *button,
                                gpointer user_data);

void on_buttonPRE1_clicked(GtkWidget *button,
                           gpointer user_data);

void on_SR_buttonSV1_clicked(GtkWidget *button,
                             gpointer user_data);

void on_buttonSupp_clicked(GtkWidget *button,
                           gpointer user_data);

void on_buttonPRE4_clicked(GtkWidget *button,
                           gpointer user_data);

void on_buttonC_clicked(GtkWidget *button,
                        gpointer user_data);

void on_button72_clicked(GtkWidget *button,
                         gpointer user_data);

void on_radiobuttonDR_toggled(GtkToggleButton *togglebutton,
                              gpointer user_data);

void on_radiobuttonG_toggled(GtkToggleButton *togglebutton,
                             gpointer user_data);

void on_buttonModifier_clicked(GtkWidget *button,
                               gpointer user_data);

void on_button73_clicked(GtkWidget *button,
                         gpointer user_data);

void on_radiobuttonC_toggled(GtkToggleButton *togglebutton,
                             gpointer user_data);

void on_treeview5_row_activated(GtkTreeView *treeview,
                                GtkTreePath *path,
                                GtkTreeViewColumn *column,
                                gpointer user_data);

void on_SR_buttonACTUALISER_clicked(GtkWidget *button,
                                    gpointer user_data);

void on_SR_buttonPRE5_clicked(GtkWidget *button,
                              gpointer user_data);

void on_SR_buttonCHERCHER_clicked(GtkWidget *button,
                                  gpointer user_data);

void on_SR_buttonPRE6_clicked(GtkWidget *button,
                              gpointer user_data);

void on_HB_buttonAE1_clicked(GtkWidget *button,
                             gpointer user_data);

void on_HB_buttonME1_clicked(GtkWidget *button,
                             gpointer user_data);

void on_HB_buttonSE1_clicked(GtkWidget *button,
                             gpointer user_data);

void on_RG_buttonS_clicked(GtkWidget *button,
                           gpointer user_data);

void on_HB_buttona2_clicked(GtkWidget *button,
                            gpointer user_data);

void on_HB_buttonm_clicked(GtkWidget *button,
                           gpointer user_data);

void on_HB_buttonPrecedent_clicked(GtkWidget *button,
                                   gpointer user_data);

void on_HB_buttonOK_AE_clicked(GtkWidget *button,
                               gpointer user_data);

void on_HB_radiobuttonAjouter_toggled(GtkToggleButton *togglebutton,
                                      gpointer user_data);

void on_HB_checkbuttonModifier_toggled(GtkToggleButton *togglebutton,
                                       gpointer user_data);

void on_HB_buttonMenu_clicked(GtkWidget *button,
                              gpointer user_data);

void on_HB_button52_clicked(GtkWidget *button,
                            gpointer user_data);

void on_HB_button51_clicked(GtkWidget *button,
                            gpointer user_data);

void on_buttonM_Re_ID_clicked(GtkWidget *button,
                              gpointer user_data);

void on_treeview1_row_activated(GtkTreeView *treeview,
                                GtkTreePath *path,
                                GtkTreeViewColumn *column,
                                gpointer user_data);

void on_HB_buttonPr_clicked(GtkWidget *button,
                            gpointer user_data);

void on_buttonS_Re_ID_clicked(GtkWidget *button,
                              gpointer user_data);

void on_HB_buttonS_clicked(GtkWidget *button,
                           gpointer user_data);

void on_RG_treeview4OBS_row_activated(GtkTreeView *treeview,
                                      GtkTreePath *path,
                                      GtkTreeViewColumn *column,
                                      gpointer user_data);

void on_RG_buttonAOBS_clicked(GtkWidget *button,
                              gpointer user_data);

void on_RG_buttonMOBS_clicked(GtkWidget *button,
                              gpointer user_data);

void on_RG_buttonDELOBS_clicked(GtkWidget *button,
                                gpointer user_data);

void on_RG_buttonADDOBS_clicked(GtkWidget *button,
                                gpointer user_data);

void on_buttongestionBV_clicked(GtkWidget *button,
                                gpointer user_data);

void on_buttonGestionObs_clicked(GtkWidget *button,
                                 gpointer user_data);

void on_buttonHome_clicked(GtkWidget *button,
                           gpointer user_data);

void on_FSBVtreeview_ACC_TV_BV_row_activated(GtkTreeView *treeview,
                                             GtkTreePath *path,
                                             GtkTreeViewColumn *column,
                                             gpointer user_data);

void on_FSBVbutton_ACC_RECH_BV_clicked(GtkWidget *objet,
                                       gpointer user_data);

void on_FSBVbuttonRetour_clicked(GtkWidget *button,
                                 gpointer user_data);

void on_RG_buttonMODifier_clicked(GtkWidget *objet_graphique,
                                  gpointer user_data);
