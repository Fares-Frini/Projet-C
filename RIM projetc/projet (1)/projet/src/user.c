   ...
   if (store == NULL)
   {
      ...
      else
      {
         ...
         while (fscanf(f, "%s %s %s %s %s %s %s %s \n ", id, idagbv, region, mnp, adresse, salle, cpt_elec, cpt_obser) != EOF)
         {
            ...
            gtk_list_store_set(store, &iter, CIN, id, NOM, idagbv, PRENOM, region, DATE, mnp, ETATSOCIAL, adresse, NATION, salle, MUNICIPALITE, cpt_elec, GOUVER, cpt_obser, IDBV, "test", -1);
         }
      }
      fclose(f);
